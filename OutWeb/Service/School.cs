    ...
    public class School
    {
        ...
        public DataTable Video_List(ref string err_msg, string video_id = "", string sort = "", string status = "", string title_query = "", string lang_id = "")
        {
            ...
            try
            {
                ...
                {
                    csql = csql + " and b1.id in (";
                    for (int i = 0; i < Array_video_id.Length; i++)
                    {
                        if (i > 0)
                        {
                            csql = csql + ",";
                        }
                        csql = csql + "@str_video_id" + i.ToString();
                    }
                    csql = csql + ") ";
                }
                ...
                if (title_query.Trim().Length > 0)
                {
                    ...
                    for (int i = 0; i < Array_title_query.Length; i++)
                    {
                        ...
                        csql = csql + " b1.C_URL like @str_title_query" + i.ToString() + " ";
                    }
                    csql = csql + ") ";
                }
                ...
                if (video_id.Trim().Length > 0)
                {
                    for (int i = 0; i < Array_video_id.Length; i++)
                    {
                        cmd.Parameters.AddWithValue("@str_video_id" + i.ToString(), Array_video_id[i]);
                    }
                }
        ...
        #region 直播新增 Video_Insert
        public string Video_Insert(string c_url = "", string is_show = "", string sort = "", string lang_id = "")
        {
            string c_msg = "";
            ...
            {
                ////========抓取sort==============================================================//
                //csql = "select (max(sort) + 1) as sort from " + video_dbf_name + " where lang_id = @lang_id";
                //cmd.CommandText = csql;
        ...
        #region 直播刪除 Video_Del
        public string Video_Del(string video_id = "")
        {
            string c_msg = "";
